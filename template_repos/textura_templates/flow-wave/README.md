...
This bundle is the complete source for the **Flow Wave** 3D scene: a single self-contained
`flow-wave.html` file plus every asset it needs (in `assets/`). It's built with Three.js and
...
```sh
npx serve .          # then open the printed URL and click flow-wave.html
