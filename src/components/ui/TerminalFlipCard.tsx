...
/**
 * The one true card , ported from the `terminal_card`
...
export interface TerminalFlipCardProps {
  ...
  external?: boolean;
  className?: string;
  /** optional 2-liner on the flip back  */
  backCaption?: string;
}
...
export function TerminalFlipCard({
  ...
  const [flipped, setFlipped] = useState(false); // touch-and-hold flip (mobile, no hover)
  ...
  // Press and hold flips to the back face (mirrors desktop hover); a quick tap
  // still navigates. If a hold happened, the click is swallowed so the hold is
  // a preview, not a navigation.
  const onTouchStart = () => {
    if (reduced) return;
  ...
  const onTouchMove = () => clearHold(); // scrolling cancels the hold
  const onTouchEnd = () => {
    clearHold();
    if (heldRef.current) setFlipped(false);
  };
