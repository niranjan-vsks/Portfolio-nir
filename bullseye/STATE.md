...
3. **Chatbot tone (Phase I — GATE).** Plumbing complete: streaming `/api/ask`, Groq path + grounded mock fallback, persona prompt, honesty guardrail, in-memory retrieval. **It is thin because `interview/*` is empty** (your voice). Fill those files and tone improves immediately. UI: `ChatPanel.tsx`.
