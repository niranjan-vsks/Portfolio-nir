...
## H. Session log
- **2026-07-13 (NOW_FIXES — full pass, Fable 5):** All items shipped + verified (44/44 verify suite green, build green 39 routes, tsc/eslint clean). (1) **System Design overhauled** (`systemDesignData.ts` rewritten as data-driven `SD_SECTIONS`): ONLY the QE platform keeps "reference architecture · how I would build it" framing; HPE RAG, Census, Loop Copilot, Saarthi, WealthOS are now shown as ACTUAL architectures (firewall names Coforge/Mphasis/Lenovo/Worktop still enforced by tests; HPE is allowed). Every project section = premium Eraser-rendered architecture image (official icons: Docker/K8s/ECS/S3/Lambda/Redis/Neo4j/Bedrock/Confluence/Jira/Azure services; PNGs in `public/architecture/*.png`, sources in Eraser file KJPT5Ekj5aOfuMeONQ9V — NOTE: Eraser free AI credits exhausted; further diagrams must use `manually_create_diagram` with freeform JSON) + interactive React Flow explorer (hover rationale, plane toggles) + system-design-interview grid (functional / non-functional / scale+capacity / tradeoffs). Click any diagram/image → fullscreen (Esc closes). Architecture write-ups added to `portfolio-assets/content/system-design/*.md` (all 5). "how I would build it" captions removed from HomeClient card + Wikify. All 6 project pages now link to /system-design#slug. (2) **HPE projects renamed + enriched**: hpe-rag-chatbot → "Enterprise Knowledge Assistant · Conversational RAG (HPE)" (status production, ~1,700 docs per resume — old 3470 figure was stale, Squidex+Confluence, 20k-concurrent design target); global-census-chatbot → "National Census Digital Assistant (HPE)" (Azure, NPR registration, birth/death reporting, APIM boundary). Search index updated. (3) **FDE**: new `guardrails-ai-safety.md` capability page (order 8; llmops bumped to 9); finetuning-rlhf-lora already existed; verify routes + search updated. (4) **Project pages**: SliderSpectra placeholder text-boxes REMOVED (slider renders only with real screenshots); prose bumped to 17px neutral-300. (5) **Starman avatar**: frameless floating figure (mix-blend-screen + radial mask), comic dialog above head, ask_niranjan pill under legs; first-visit-per-session auto-open → dialog hides at 5s → collapses at 10s; click-anywhere dismisses dialog. (6) **Mind map**: initial dolly 0.7→0.48 (labels readable on arrival, verified); brain intro plays only on FIRST /map visit per session and is ALWAYS skipped on ?node= deep links; /system-design tag chips now deep-link to /map?node=<id> (TAG_NODE map in SystemDesignClient). **Known env issue this session:** claude-mem plugin worker died mid-session (blocks Read of images; hooks spam "worker unreachable") — visual verification done via Playwright MCP screenshots instead. **Remaining:** R11 Lighthouse audit, chatbot tone gate (blocked on interview/*.md), custom domain aliasing, redeploy to Vercel prod (this session's work NOT yet deployed).
